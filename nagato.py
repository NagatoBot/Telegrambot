...
# ==========================================
# TEMPLATES (WITH FIXED CLOCK PARSER & PROPER BUTTONS)
# ==========================================
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <!-- Navigation Drawer with Icons Restored -->
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <div class="glass-card rounded-2xl p-4 flex flex-col justify-between neon-border-pink">
                        <div class="flex justify-between items-center mb-1">
                            <span id="expiryBadge" class="text-[10px] font-tech px-2 py-0.5 rounded-md bg-fuchsia-500/20 text-fuchsia-300 border border-fuchsia-500/40">ACTIVE</span>
                        </div>
                        <div>
                            <span id="countdownTimer" class="font-tech text-base md:text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-cyan-300">Calculating...</span>
                            <p class="text-[10px] text-purple-400 mt-0.5 font-mono">Valid: {{ panel_expiry }}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody class="divide-y divide-purple-900/30 text-slate-300">
                                ...
                                <tr>
                                    ...
                                    <td class="py-2.5 px-3 text-right">
                                        <span class="px-2 py-0.5 rounded text-[10px] uppercase font-tech {{ 'text-emerald-400 bg-emerald-500/10' if st == 'approved' else 'text-amber-400 bg-amber-500/10' }}">{{ st }}</span>
                                    </td>
                                </tr>
                                {% else %}
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs font-mono">
                            ...
                            <tbody class="divide-y divide-purple-900/30 text-slate-300">
                                ...
                                {% else %}
                                <tr><td colspan="6" class="py-8 text-center text-purple-400">No orders recorded in database.</td></tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="w-full text-left text-xs font-mono text-slate-300 border-collapse">
                            ...
                            <tbody class="divide-y divide-purple-900/30">
                                ...
                                <tr class="hover:bg-purple-900/20 transition">
                                    ...
                                    <!-- Independent Update Form with distinct input cells -->
                                    ...
                                    <!-- Separate, Dedicated Delete Column & Form -->
                                    <td class="p-3 text-center align-middle">
                                        <form method="POST" action="/admin/plans/delete" onsubmit="return confirm('Permanently delete {{ name }} ({{ pid }})?');">
                                            <input type="hidden" name="plan_id" value="{{ pid }}">
                                            <button type="submit" class="bg-rose-500/20 hover:bg-rose-600 text-rose-400 hover:text-white font-tech text-[10px] px-3.5 py-1.5 rounded-lg uppercase tracking-wider transition">
                                                Delete
                                            </button>
                                        </form>
                                    </td>
    ...
    <!-- Reliable, Mobile-Compatible Vanilla Script -->
    <script>
        ...
        // Universal parser: Splits numbers directly to avoid mobile browser parsing bugs
        (function() {
            var timerEl = document.getElementById("countdownTimer");
            var badgeEl = document.getElementById("expiryBadge");
            var rawExpiry = "{{ panel_expiry }}";
            if (!timerEl || !rawExpiry) return;

            var parts = rawExpiry.match(/\\d+/g);
            var expiryDate;
            if (parts && parts.length >= 6) {
                expiryDate = new Date(parts[0], parts[1] - 1, parts[2], parts[3], parts[4], parts[5]).getTime();
            } else {
                expiryDate = new Date(rawExpiry.replace(' ', 'T')).getTime();
            }

            function runClock() {
                if (isNaN(expiryDate)) {
                    timerEl.innerText = "ACTIVE";
                    return;
                }
