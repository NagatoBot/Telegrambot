...
async def detect_payee_name_from_upi(upi_id: str) -> str:
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=3.0), headers=headers) as s:
...
async def process_plan_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    caption = (
        ...
        f"UPI ID: {upi_id}\n\n"
        f"1. Scan QR and pay\n"
        f"2. Tap CHECK PAYMENT and upload screenshot"
    )
...
<head>
    ...
    <title>Nagato Panel - Login</title>
...
<body class="bg-[#0b0c16] text-slate-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 mb-3 shadow-lg shadow-cyan-500/10 text-2xl">
                &#128640;
            </div>
            <h1 class="text-2xl font-bold tracking-tight text-white">Nagato Panel</h1>
            <p class="text-xs text-slate-400 mt-1">Authenticate to manage your payments &amp; telegram bot</p>
        </div>

        <div class="bg-[#101223]/90 backdrop-blur-xl border border-slate-800 rounded-2xl p-7 shadow-2xl space-y-4">
            {% if error %}
            <div class="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-medium">
                {{ error }}
            </div>
            {% endif %}
            ...
            <div class="pt-3 border-t border-slate-800/80 text-center">
                <a href="https://t.me/NAGATOxOWNER" target="_blank" rel="noopener noreferrer"
                   class="inline-flex items-center justify-center gap-1.5 text-xs text-cyan-400 hover:text-cyan-300 font-medium transition py-1">
        ...
        <p class="text-center text-slate-600 text-xs mt-6">&copy; Nagato Panel. All rights reserved.</p>
    </div>
</body>
</html>"""
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="font-bold text-base text-cyan-400 tracking-tight">Nagato Panel</span>
                </div>
            ...
            <nav class="space-y-1 text-xs">
                ...
                <button onclick="switchTab('tab-orders')" id="nav-tab-orders" class="nav-btn w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-slate-400 hover:bg-[#14172d] hover:text-white transition">
                ...
                <button onclick="switchTab('tab-users')" id="nav-tab-users" class="nav-btn w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-slate-400 hover:bg-[#14172d] hover:text-white transition">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    Manage Users
                </button>
                <button onclick="switchTab('tab-broadcast')" id="nav-tab-broadcast" class="nav-btn w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-slate-400 hover:bg-[#14172d] hover:text-white transition">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="tab-dashboard" class="tab-content space-y-6">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <!-- 30-Day Auto Countdown Card -->
                    <div class="card-glow rounded-2xl p-4 flex flex-col justify-between">
                        <div class="flex items-center justify-between mb-2">
                            <span class="p-2 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </span>
                            <span id="expiryBadge" class="text-[10px] font-mono px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                                Active
                            </span>
                        </div>
                        <div>
                            <span id="countdownTimer" class="text-lg md:text-xl font-extrabold text-white tracking-tight font-mono">
                                Calculating...
                            </span>
                            <p class="text-[11px] text-slate-400 font-medium mt-0.5">
                                Valid till: <span class="text-slate-200">{{ panel_expiry }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-glow rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1a1d36] pb-3">
                        <div>
                            <span class="text-xs text-cyan-400 font-medium">Gateway: Direct UPI QR Code</span>
                            <h3 class="text-lg font-bold text-white tracking-tight">Recent orders</h3>
                        </div>
                        <button onclick="switchTab('tab-orders')" class="text-xs text-cyan-400 hover:text-cyan-300 font-medium">View All Orders &rarr;</button>
                    </div>
            ...
            <div id="tab-orders" class="tab-content space-y-6 hidden">
                <div class="card-glow rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1a1d36] pb-3">
                        <div>
                            <span class="text-xs text-cyan-400 font-medium">Nagato Panel Log</span>
                            <h3 class="text-lg font-bold text-white tracking-tight">Customer Orders</h3>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left text-xs">
                            ...
                            <tbody class="divide-y divide-[#15172d] text-slate-300 font-medium">
                                ...
                                <tr class="hover:bg-[#12142a]/40 transition">
                                    ...
                                    <td class="py-3 px-3 text-white font-medium">{{ item }}</td>
                                    <td class="py-3 px-3 font-semibold text-white">&#8377;{{ "%.2f"|format(amt) }}</td>
                                    <td class="py-3 px-3 text-slate-400 text-[11px]">{{ dt }}</td>
                                    <td class="py-3 px-3 text-right">
                                        {% if st == 'approved' %}
                                        <span class="px-2.5 py-1 rounded-md text-[10px] font-bold bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">Paid</span>
                                        {% elif st == 'pending' %}
                                ...
                                {% else %}
                                <tr>
                                    <td colspan="6" class="py-12 text-center text-slate-500 text-xs">No orders recorded in database.</td>
                                </tr>
                                {% endfor %}
                            </tbody>
                        </table>
            ...
            <div id="tab-users" class="tab-content space-y-6 hidden">
                <div class="card-glow rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1a1d36] pb-3">
                        <div>
                            <span class="text-xs text-cyan-400 font-medium">Customer Management</span>
                            <h3 class="text-lg font-bold text-white tracking-tight">Registered Users</h3>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            ...
                            <tbody class="divide-y divide-[#15172d] text-slate-300 font-medium">
                                ...
                                <tr class="hover:bg-[#12142a]/40 transition">
                                    <td class="py-3 px-3 font-mono text-cyan-400">
                                        {{ u_id }}
                                        <span class="block text-[10px] text-white font-normal">{{ u_fname }}</span>
                                    </td>
                                    <td class="py-3 px-3">
                                        {% if u_uname != 'N/A' %}
                                        <a href="https://t.me/{{ u_uname }}" target="_blank" class="text-cyan-400 hover:underline">@{{ u_uname }}</a>
                                        {% else %}
                                        <span class="text-slate-500">None</span>
                                        {% endif %}
                                    </td>
                                    <td class="py-3 px-3 text-slate-400 text-[11px]">{{ u_joined }}</td>
                                    <td class="py-3 px-3">
                                        <form method="POST" action="/admin/users/subscription" class="flex items-center gap-1.5">
                                            ...
                                            <select name="plan_name" class="bg-[#080914] border border-[#1a1d36] rounded-lg px-2 py-1 text-[11px] text-white focus:outline-none focus:border-cyan-500">
                                            ...
                                            <button type="submit" class="bg-[#14172d] hover:bg-cyan-600 text-white px-2 py-1 rounded-lg text-[10px] transition">Set</button>
                                        </form>
                                    </td>
                                    <td class="py-3 px-3 text-right">
                                        <form method="POST" action="/admin/users/ban">
                                            ...
                                            {% if u_banned %}
                                            <button type="submit" class="bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500 text-emerald-400 hover:text-white px-2.5 py-1 rounded-lg text-xs transition">Unban</button>
                                            {% else %}
                                            <button type="submit" class="bg-rose-500/10 border border-rose-500/30 hover:bg-rose-500 text-rose-400 hover:text-white px-2.5 py-1 rounded-lg text-xs transition">Ban</button>
                                            {% endif %}
                                        </form>
                                    </td>
                                </tr>
                                {% else %}
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-slate-500 text-xs">No registered users in database yet.</td>
                                </tr>
                                {% endfor %}
                            </tbody>
                        </table>
            ...
            <div id="tab-broadcast" class="tab-content space-y-6 hidden">
                <form method="POST" action="/admin/broadcast/send" class="card-glow rounded-2xl p-6 space-y-5">
                    <div class="flex items-center gap-3 border-b border-[#1a1d36] pb-4">
                        <div class="w-9 h-9 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-white tracking-tight">Mass Broadcast</h3>
                            <p class="text-xs text-slate-400">Instantly deliver an announcement or alert to all unbanned users in your database.</p>
                        </div>
                    </div>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Message Body</label>
                            <textarea name="broadcast_message" rows="5" required placeholder="Type your update or announcement here..."
                                      class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500 transition"></textarea>
                            <p class="text-[11px] text-slate-500 mt-1.5">Includes automatic flood prevention and rate limits.</p>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Optional Banner Image URL</label>
                            ...
                            <p class="text-[11px] text-slate-500 mt-1.5">Leave blank to broadcast text only.</p>
                        </div>
                    </div>

                    <button type="submit" onclick="return confirm('Broadcast this message to all bot users?');"
                            class="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold px-6 py-2.5 rounded-xl text-xs transition shadow-lg shadow-cyan-500/20">
                        &#128227; Send Broadcast
                    </button>
                </form>
            </div>
            ...
            <div id="tab-settings" class="tab-content space-y-6 hidden">
                <form method="POST" action="/admin/settings/upi" class="card-glow rounded-2xl p-6 space-y-5">
                    <div class="flex items-center gap-3 border-b border-[#1a1d36] pb-4">
                        <div class="w-9 h-9 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-white tracking-tight">UPI Payment Setup</h3>
                            <p class="text-xs text-slate-400">Provide your UPI ID and registered account payee name for QR creation.</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                <label class="block text-xs font-semibold text-slate-300">UPI ID *</label>
                                <button type="button" onclick="autoDetectName()" class="text-xs text-cyan-400 hover:text-cyan-300 font-medium flex items-center gap-1">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-2">Payee Registered Name</label>
                            <input type="text" id="payee_name_input" name="payee_name" value="{{ payee_name }}" required placeholder="Account Holder Name"
                                   class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500 transition">
                            <p id="detectStatus" class="text-[11px] text-slate-500 mt-1.5">You can type manually or tap 'Auto-detect Name' after changing the UPI ID.</p>
                        </div>

                        <div class="pt-2">
                            <label class="block text-xs font-semibold text-slate-300 mb-2">Admin Telegram Chat ID (Order Alert Recipient)</label>
                ...
                <form method="POST" action="/admin/password/update" class="card-glow rounded-2xl p-6 space-y-4">
                    ...
                    <div class="pt-2">
                        ...
                        <p class="text-[11px] text-slate-500 mt-2">Logged in as <b>Nagato</b>.</p>
                    </div>
                </form>
            </div>
            ...
            <div id="tab-plans" class="tab-content space-y-5 hidden">
                <div class="card-glow rounded-2xl p-6 space-y-4">
                    <h3 class="text-base font-bold text-white tracking-tight">Add New Membership Plan</h3>
                    <form method="POST" action="/admin/plans/add" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase">Plan ID (Unique)</label>
                            <input type="text" name="plan_id" required placeholder="e.g. plan_9" class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500">
                        </div>
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase">Plan Title</label>
                            <input type="text" name="name" required placeholder="e.g. ULTRA VIP" class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500">
                        </div>
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase">Price (Rs.)</label>
                            <input type="number" step="any" name="amount" required placeholder="e.g. 499" class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500">
                        </div>
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase">Validity</label>
                            <input type="text" name="validity" required placeholder="e.g. 60 Days" class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-cyan-500">
                        </div>
                        <div>
                            <button type="submit" class="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-2 rounded-xl text-xs transition">
                                + Add Plan
                            </button>
                        </div>
                    </form>
                                    ...
                                    <form method="POST" action="/admin/plans/update">
                                        ...
                                        <td class="p-3 text-right flex items-center justify-end gap-2">
                                            ...
                                            <form method="POST" action="/admin/plans/delete" onsubmit="return confirm('Delete this plan?');">
                                                <input type="hidden" name="plan_id" value="{{ pid }}">
                                                <button type="submit" class="bg-rose-500/10 border border-rose-500/30 hover:bg-rose-500 text-rose-400 hover:text-white px-2.5 py-1 rounded-lg text-xs transition">Delete</button>
                                            </form>
                                        </td>
                                </tr>
                                {% endfor %}
    ...
    <script>
        ...
        async function autoDetectName() {
            ...
            if (!upiId || !upiId.includes('@')) {
                ...
                statusEl.className = "text-[11px] text-rose-400 mt-1.5";
                return;
            }

            spinner.classList.remove('hidden');
            statusEl.innerText = "Querying directory for registered account name...";
            statusEl.className = "text-[11px] text-cyan-400 mt-1.5";
        ...
        function updateCountdown() {
            ...
            if (isNaN(distance) || distance <= 0) {
                ...
                timerEl.className = "text-lg md:text-xl font-extrabold text-rose-400 tracking-tight font-mono";
                badgeEl.innerText = "Expired";
                badgeEl.className = "text-[10px] font-mono px-2 py-0.5 rounded-md bg-rose-500/10 text-rose-400 border border-rose-500/30";
                return;
            }
