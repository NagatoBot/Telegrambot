...
async def detect_payee_name_from_upi(upi_id: str) -> str:
    ...
    try:
        url = f"https://p-pay.paytm.com/v1/api/verifyVpa?vpa={urllib.parse.quote(upi_id)}"
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=4.0)) as s:
            async with s.get(url) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    name = data.get("name") or data.get("payeeName") or data.get("accountName")
                    if name:
                        return name.strip()
    except Exception:
        pass
    ...
    return await get_setting("payee_name")
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden">
                ...
                <form method="POST" action="/admin/settings/upi" class="card-glow rounded-2xl p-6 space-y-5">
                    <div class="flex items-center gap-3 border-b border-[#1a1d36] pb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400">Provide your UPI ID. Payee name will be auto-detected for QR creation.</p>
                        </div>
                    </div>
    ...
    <script>
        ...
        async function autoDetectName() {
            ...
            try {
                ...
                if (data.name && data.name.trim().length > 0) {
                    ...
                    statusEl.innerText = "Could not auto-detect automatically. You can enter your name manually above.";
        ...
        // Live 30-Day Auto Countdown Engine
...
async def api_detect_upi_name(upi_id: str, is_auth: bool = Depends(require_admin)):
    ...
    return JSONResponse(content={"upi_id": upi_id, "name": name})
