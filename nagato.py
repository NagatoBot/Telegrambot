...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        defaults = {
            ...
            "panel_expiry": "2026-09-16 14:48:00",
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <div id="tab-settings" class="tab-content space-y-6 hidden">
                ...
                <!-- Expiry Date Setting Card -->
                <form method="POST" action="/admin/settings/expiry" class="card-glow rounded-2xl p-6 space-y-4">
                    <div class="flex items-center gap-3 border-b border-[#1a1d36] pb-3">
                        ...
                        <h3 class="text-base font-bold text-white">License &amp; Expiry Configuration</h3>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-2">Website Expiry Timestamp (YYYY-MM-DD HH:MM:SS)</label>
                        <input type="text" name="panel_expiry" value="{{ panel_expiry }}" required placeholder="2026-09-16 14:48:00"
                               class="w-full bg-[#080914] border border-[#1a1d36] rounded-xl px-4 py-2.5 text-sm text-white font-mono focus:outline-none focus:border-cyan-500 transition">
                        <p class="text-[11px] text-slate-500 mt-1.5">Controls the real-time countdown timer shown on the live dashboard.</p>
                    </div>
...
async def save_upi_setup(
    ...
    return RedirectResponse(url="/?message=UPI+and+payee+details+saved+successfully", status_code=status.HTTP_303_SEE_OTHER)


@app.post("/admin/settings/expiry")
async def save_expiry_timestamp(
    panel_expiry: str = Form(...),
    is_auth: bool = Depends(require_admin),
):
    await update_setting("panel_expiry", panel_expiry.strip())
    return RedirectResponse(url="/?message=License+expiry+date+updated+successfully", status_code=status.HTTP_303_SEE_OTHER)
