...
# Safe ban handler preventing 500 crashes
...
async def handle_user_ban_toggle(
    ...
    try:
        await set_user_ban_status(int(user_id), int(status))
        action_text = "banned" if int(status) == 1 else "unbanned"
...
async def handle_user_subscription_change(
    ...
    try:
        await update_user_subscription(int(user_id), plan_name.strip())
        if manager.bot:
            if plan_name == "Free":
                try:
                    await manager.bot.send_message(int(user_id), "Your premium subscription has ended. You are now on the Free tier.")
                except Exception:
                    pass
            else:
                await notify_payment_approved(int(user_id), plan_name.strip())
