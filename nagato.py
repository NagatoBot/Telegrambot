...
# ==========================================
# DYNAMIC BOT CONTROLLER (OPTIMIZED POLLING)
# ==========================================
class BotManager:
    def __init__(self):
    ...
    async def start(self, token: str):
        ...
        # Increased connection pool and lower timeout for responsiveness
        self.session = AiohttpSession(timeout=25.0)
        self.bot = Bot(
            token=token, 
        ...
        try:
            # Purge pending updates to unstick backlog/flood
        ...
        async def runner():
            try:
                logging.info("[BotManager] Polling starting with clean update queue...")
...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        defaults = {
            ...
            # Reliable, non-blocking image CDN
...
async def send_welcome_flow(chat_id: int):
    """Guaranteed non-blocking welcome delivery with instant fallback."""
    if not manager.bot:
        return
    ...
    if photo_url and photo_url.startswith(("http://", "https://", "AgAC")):
        try:
            # 2.5s hard timeout prevents Telegram/Unsplash connection hanging
            ...
            logging.warning(f"[Welcome] Photo timeout or error, sending text fallback: {e}")
...
async def process_plan_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    caption = (
        f"\U0001F4F2 \u1D1C\u1D18\u026A \u1D18\u1D00\u028F\u1D0D\u1D07\u0274\u1D1B\n\n"
        f"\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n"
        f"\U0001F4E6 \u1D18\u029F\u1D00\u0274: {plan_name}\n"
        f"\U0001F4B0 \u1D00\u1D0D\u1D0F\u1D1C\u0274\u1D1B: \u20B9{amount:.2f}\n"
        f"\u23F3 \u1D20\u1D00\u029F\u026A\u1D1B\u028F: {validity}\n"
        f"\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n\n"
        f"\U0001F464 \u0274\u1D00\u1D0D\u1D07: {payee}\n"
        f"\U0001F4F1 UPI ID: <code>{upi_id}</code>\n\n"
        f"\U0001F4CB s\u1D1B\u1D07\u1D18s:\n"
        f"1\uFE0F\u20E3 s\u1D04\u1D00\u0274 \u1D1B\u029C\u1D07 Q\u0280 \u1D04\u1D0F\u1D05\u1D07 \u1D00\u0299\u1D0F\u1D20\u1D07\n"
        f"2\uFE0F\u20E3 \u20B9 \u1D00\u1D0D\u1D0F\u1D1C\u0274\u1D1B \u1D00\u0274\u1D05 \u0274\u1D0F\u1D1B\u1D07 \u1D21\u026A\u029F\u029F \u0299\u1D07 \uA7F0\u026A\u029F\u029F\u1D07\u1D05 \u1D00\u1D1C\u1D1B\u1D0F\u1D0D\u1D00\u1D1B\u026A\u1D04\u1D00\u029F\u029F\u028F\n"
        f"3\uFE0F\u20E3 \u1D07\u0274\u1D1B\u1D07\u0280 \u028F\u1D0F\u1D1C\u0280 \u1D18\u026A\u0274 \u1D00\u0274\u1D05 \u1D04\u1D0F\u1D0D\u1D18\u029F\u1D07\u1D1B\u1D07 \u1D1B\u029C\u1D07 \u1D18\u1D00\u028F\u1D0D\u1D07\u0274\u1D1B\n"
        f"4\uFE0F\u20E3 \u1D1B\u1D00\u1D0B\u1D07 \u1D00 s\u1D04\u0280\u1D07\u1D07\u0274s\u029C\u1D0F\u1D1B \u1D00\u0274\u1D05 \u1D1B\u1D00\u1D18 \u1D04\u029C\u1D07\u1D04\u1D0B \u1D18\u1D00\u028F\u1D0D\u1D07\u0274\u1D1B \u2705"
    )
...
async def handle_check_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    msg = await manager.bot.send_message(
        chat_id=callback.message.chat.id,
        text=(
            "\U0001F4F8 s\u1D07\u0274\u1D05 \u1D18\u1D00\u028F\u1D0D\u1D07\u0274\u1D1B s\u1D04\u0280\u1D07\u1D07\u0274s\u029C\u1D0F\u1D1B\n\n"
            "\u2705 s\u1D07\u0274\u1D05 \u1D1B\u029C\u1D07 s\u1D04\u0280\u1D07\u1D07\u0274s\u029C\u1D0F\u1D1B \u029C\u1D07\u0280\u1D07 \u1D00\uA7F0\u1D1B\u1D07\u0280 \u1D04\u1D0F\u1D0D\u1D18\u029F\u1D07\u1D1B\u026A\u0274\u0192 \u1D1B\u029C\u1D07 \u1D1C\u1D18\u026A \u1D18\u1D00\u028F\u1D0D\u1D07\u0274\u1D1B.\n\n"
            "\u26A0\uFE0F \u1D0F\u0274\u029F\u028F \u1D00\u0274 \u026A\u1D0D\u1D00\u0192\u1D07 \u1D0F\u0280 s\u1D04\u0280\u1D07\u1D07\u0274s\u029C\u1D0F\u1D1B \u026As \u1D00\u1D04\u1D04\u1D07\u1D18\u1D1B\u1D07\u1D05\n\n"
...
async def invalid_proof(message: types.Message, state: FSMContext):
    if message.text == "/cancel":
        ...
        return
    msg = await message.answer(
        "\u26A0\uFE0F <b>\u1D0F\u0274\u029F\u028F \u1D00\u0274 \u026A\u1D0D\u1D00\u0192\u1D07 \u1D0F\u0280 s\u1D04\u0280\u1D07\u1D07\u0274s\u029C\u1D0F\u1D1B \u026As \u1D00\u1D04\u1D04\u1D07\u1D18\u1D1B\u1D07\u1D05</b>\n\nPlease upload your payment screenshot image directly."
    )
...
# ==========================================
# TEMPLATES (EXACT MATCH NOISY/NAGATO LOGIN & CYBERPUNK THEME)
# ==========================================
